...
export const ScriptCard: React.FC<ScriptCardProps> = ({ plan, personaType }) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden transition-all hover:shadow-2xl">
        ...
        <div className="flex flex-col gap-2 mb-6">
            ...
            <div className="flex justify-between items-start gap-6">
                <div className="space-y-2">
                    <h2 className="text-3xl font-bold text-slate-900 leading-tight font-serif-jp tracking-tight">{plan.title_ja}</h2>
                    <p className="text-sm font-semibold text-indigo-500/80">{plan.title_kr}</p>
                </div>
                <button
                    onClick={() => handleCopy(plan.title_ja, 'title_ja')}
                ...
                >
                    {copied === 'title_ja' ? <Check size={20} /> : <Copy size={20} />}
                </button>
            </div>
        </div>
      ...
      {/* Korean Translation (Hidden by default for focus) */}
      <div className="px-8 py-5 border-b border-slate-100 bg-slate-50/50">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-slate-500 group-hover:text-indigo-600 uppercase tracking-widest transition-colors">Visual Logic Summary (KR)</h3>
          </div>
          {showKr ? <ChevronUp size={18} className="text-slate-300" /> : <ChevronDown size={18} className="text-slate-300" />}
        </button>
        {showKr && (
      ...
      <div className="p-8 bg-slate-900">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-black text-indigo-400 uppercase tracking-widest">Narration Master Script</h3>
            </div>
            <button
              onClick={() => handleCopy(plan.script_ja, 'script_ja')}
              className="p-2 text-slate-500 hover:text-white transition-colors"
            >
              {copied === 'script_ja' ? <Check size={20} /> : <Copy size={20} />}
            </button>
          </div>
