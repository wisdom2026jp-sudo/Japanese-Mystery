...
const MYSTERY_PRESETS = [
  { title: "きさらぎ駅", title_kr: "키사라기 역", query: "存在しない駅『きさらぎ駅』の不可解な真相과 이세계 괴담" },
  { title: "犬鳴トンネル", title_kr: "이누나키 터널", query: "일본 최흉의 심령 스팟, 이누나키 터널의 역사와 괴담" },
  { title: "口裂け女", title_kr: "빨간 마스크", query: "1970년대 일본을 흔든 구네구네와 입 찢어진 여자의 정체" },
  { title: "八尺様", title_kr: "팔척귀신", query: "홀리면 끝장, 8척의 거구 하치샤쿠사마의 공포" }
];
    ...
    <aside className="w-full md:w-[420px] bg-white border-r border-slate-200 p-8 flex flex-col z-10 shadow-xl overflow-y-auto custom-scrollbar">
      <div className="mb-10">
        <div className="flex items-center gap-3 text-indigo-600 mb-2">
          ...
          <span className="font-black text-2xl tracking-tighter uppercase">Pro AI Studio</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-2 py-0.5 bg-slate-900 text-white text-[9px] font-black rounded uppercase tracking-widest">Enterprise Pro</span>
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">V3.5 KR/JP Sync</p>
        </div>
      </div>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="mt-10 bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-sm">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                      {plan.hookImageUrl && (
                        <div className="aspect-[9/16] rounded-2xl bg-slate-100 overflow-hidden relative group border-4 border-indigo-600 shadow-lg">
                          <img src={plan.hookImageUrl} className="w-full h-full object-cover" />
                          <div className="absolute inset-0 bg-indigo-600/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                            <span className="text-[10px] text-white font-black px-2 py-1 bg-black rounded uppercase tracking-widest">Master Hook</span>
                          </div>
                        </div>
                      )}
                      {plan.storyImageUrls?.map((url, i) => (
                        <div key={i} className="aspect-[9/16] rounded-2xl bg-slate-100 overflow-hidden relative group border-2 border-slate-100 hover:border-indigo-400 transition-all">
                          <img src={url} className="w-full h-full object-cover" />
                          <div className="absolute top-2 right-2 bg-black/70 text-white text-[9px] px-2 py-0.5 rounded-lg font-black backdrop-blur-sm">#{i + 1}</div>
                        </div>
                      ))}
                    </div>
                  </div>
